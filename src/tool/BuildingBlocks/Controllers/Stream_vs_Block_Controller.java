...
public class Stream_vs_Block_Controller extends BuildingBlockController {
    ...
    final static Paragraph streamPara= new Paragraph("Stream cipher is when the message is transformed bit by bit, or character by character. A key is given to an encryption algorithm and the algorithm is applied to each bit "+
    ...
    final static Paragraph blockPara= new Paragraph("Block cipher is when a message is divided into same sized block chunks. If the message doesn't exactly fit into the block then padding can be added, in this case the padding "+
    "is X but it depends on the cipher. The encryption/decryption algorithms are applied to the blocks. This is good for when you know how big your message is, and can also provide message authorisation, but block "+
